# AGENTS.md — Notes for AI agents (gregMod.MoreServers)

Repo: gregMod.MoreServers · License: Apache-2.0 · Version: see `VERSION` (1.0.12).

MelonMod for Data Center. Adds extra server variants via a data-driven registry.
...
4. **Verify changes:** before reporting done, build the mod (`dotnet build gregMod.MoreServers.csproj -c Release` or `./build.sh MoreServers` from `ModRepositories/`).
...
- Deploy only with `./build.sh MoreServers --deploy`.

## Hard rules

- New servers go through `ModuleRegistry` + `ModuleDefinition` (data-driven);
...
- Catalog patches stay observational plus registry-driven injection; keep them
  in sync with the sibling MoreModules/MoreSpools/RealisticModules patterns.
